...
# 读取数据（先按字符串读取，避免类型错误）
df_raw = pd.read_csv(data_path, header=None, dtype=str)  # 先按字符串读取，防止数值解析错误
...
def clean_length_data(df_raw):
    ...
    print(
        f"无效长度（≤0）行数：{len(length_series) - len(valid_lengths) - length_series.isna().sum()}"
    )
...
n, bins, patches = ax.hist(
    plot_lengths, bins=num_bins, color="#2E86AB", alpha=0.8, edgecolor="white", linewidth=0.5
)

# 图表标注优化
ax.set_title(
    f"Sequence Length Distribution (Showing 99% data, Total: {len(lengths):,})", fontsize=14, pad=20
)
