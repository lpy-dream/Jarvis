...
model = EsmForSequenceClassification.from_pretrained(MODEL_NAME, num_labels=2, cache_dir="./model")
...
def plot_history(log_history, save_path):
    ...
    if len(df) > 1:
        ...
        if restart_points:
            ...
            print(
                f"检测到历史残留日志，正在截取最后一次训练记录 (从索引 {last_restart_index} 开始)..."
            )
    ...
    plt.subplot(1, 2, 1)
    plt.plot(
        train_logs["epoch"], train_logs["loss"], label="Training Loss", alpha=0.8
    )  # 加点透明度
    if len(eval_logs) > 0:
        plt.plot(
            eval_logs["epoch"],
