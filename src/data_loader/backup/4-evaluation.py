...
def get_args():
    ...
    # 必需参数
    parser.add_argument(
        "-i", "--input_fasta", required=True, help="Path to the test FASTA file (e.g., test.fasta)"
    )
    parser.add_argument(
        "-m",
        "--lora_model",
    ...
    parser.add_argument(
        "--max_len", type=int, default=1024, help="Maximum sequence length for tokenization"
    )
    parser.add_argument(
        "--cache_dir", default="./model", help="Directory to cache downloaded base models"
    )
...
def main():
    ...
    base_model = EsmForSequenceClassification.from_pretrained(
        args.base_model, num_labels=2, cache_dir=args.cache_dir
    )
    ...
    try:
        ...
        print(
            "Make sure the path points to the folder containing adapter_config.json and adapter_model.bin"
        )
        return
    ...
    test_dataset = ProteinDataset(
        args.input_fasta, tokenizer, max_len=args.max_len
    )  # 直接返回模型需要的字典型数据
    ...
    if len(valid_indices) > 0:
        ...
        print("\nDetailed Report:")
        print(
            classification_report(clean_labels, clean_preds, target_names=["Negative", "Positive"])
        )
    else:
        print(
