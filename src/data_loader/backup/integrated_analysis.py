...
class ModelAnalyzer:
    ...
    def _load_model(self):
        ...
        self.tokenizer = EsmTokenizer.from_pretrained(
            self.args.base_model, cache_dir=self.args.cache_dir
        )
    ...
    def load_data(self):
        ...
        print(
            f"[Data] Label distribution: Positive={labels.count(1)}, "
    ...
    def run_inference(self):
        ...
        # 限制样本数量（用于t-SNE）
        max_samples = (
            self.args.tsne_samples if self.args.mode in ["tsne", "full"] else len(sequences)
        )
        sample_count = 0
    ...
    def visualize_attention(self):
        ...
        # 选择高置信度正样本进行可视化
        positive_samples = [
            att for att in self.attentions if att["pred"] == 1 and att["prob"] > 0.9
        ]
        ...
        for i in range(num_vis):
            ...
            self._plot_attention_heatmap(
                sample["seq"], sample["attention"], sample["id"], sample["prob"]
            )
...
class SimpleDataset(Dataset):
    ...
    def __getitem__(self, idx):
        return {"seq": self.sequences[idx], "label": self.labels[idx], "id": self.ids[idx]}
...
def parse_args():
    ...
    parser.add_argument(
        "-m", "--lora_model", required=True, help="Path to trained LoRA model directory"
    )
    ...
    parser.add_argument("--cache_dir", default="./model", help="Cache directory for models")
    ...
    # ===== 可视化配置 =====
    parser.add_argument(
        "--num_attention_plots", type=int, default=3, help="Number of attention maps to generate"
    )
    parser.add_argument(
        "--tsne_samples", type=int, default=3000, help="Maximum samples for t-SNE (to save time)"
    )
