...
def parse_args():
    ...
    parser.add_argument(
        "-a", "--keywords", required=True, help="文件 A：包含关键词的列表 (例如: Cressdnaviricota)"
    )
    parser.add_argument("-b", "--target_file", required=True, help="文件 B：被搜索的目标文件")
    ...
    parser.add_argument(
        "-v", "--invert", action="store_true", help="反选模式：输出【不包含】任何关键词的行。"
    )
    # --- 新增参数 ---
    parser.add_argument(
        "-i",
...
def search_and_write(file_b, keywords, output_file, mode, invert, ignore_case):
    ...
    try:
        with (
            open(file_b, encoding="utf-8", errors="ignore") as f_in,
...
def main():
    ...
    search_and_write(
        args.target_file, keys, args.output, args.mode, args.invert, args.ignore_case  # 传入新参数
    )
