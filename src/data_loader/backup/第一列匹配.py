...
def parse_args():
    ...
    parser.add_argument("-a", "--input_a", required=True, help="输入文件 A (主文件)")
    parser.add_argument("-b", "--input_b", required=True, help="输入文件 B (提供注释信息)")
...
def process_merge(file_a, annotation_map, output_file):
    ...
    try:
        with (
            open(file_a, encoding="utf-8", errors="ignore") as f_in,
