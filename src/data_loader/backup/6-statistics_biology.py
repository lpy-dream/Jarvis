...
def get_args():
    ...
    parser.add_argument(
        "-m", "--lora_model", required=True, help="Path to the trained LoRA model directory"
    )
    ...
    parser.add_argument(
        "--base_model", default="facebook/esm2_t33_650M_UR50D", help="Hugging Face base model name"
    )
    parser.add_argument(
        "--tsne_samples",
        type=int,
...
class AnalysisDataset(Dataset):
    def __init__(self, fasta_file):
        ...
        for record in SeqIO.parse(fasta_file, "fasta"):
            ...
            self.data.append({"id": record.id, "seq": str(record.seq), "label": label})
