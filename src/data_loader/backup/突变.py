...
def parse_arguments():
    ...
    # 必需参数
    parser.add_argument(
        "-i", "--input", required=True, help="输入 FASTA 文件路径 (例如: positives.fasta)"
    )
    parser.add_argument(
        "-o", "--output", required=True, help="输出 FASTA 文件路径 (例如: positives_aug.fasta)"
    )
    ...
    parser.add_argument("--seed", type=int, default=42, help="随机种子，保证结果可复现 (默认: 42)")
