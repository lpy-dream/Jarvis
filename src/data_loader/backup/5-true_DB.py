...
def collate_fn(batch, tokenizer):  # <--- 新增 tokenizer 参数
    ...
    # 使用传入的 tokenizer
    encodings = tokenizer(
        sequences, padding=True, truncation=True, max_length=1024, return_tensors="pt"
    )
...
def main():
    ...
    base_model = EsmForSequenceClassification.from_pretrained(
        BASE_MODEL, num_labels=2, output_attentions=True  # <--- 关键！开启上帝视角
    )
                ...
                for i in range(len(ids)):
                    ...
                    if preds[i] == 1 and vis_count < NUM_VISUALIZATIONS:
                        vis_save_path = os.path.join(
                            OUTPUT_DIR, f"attention_{ids[i].replace('|','_')}.png"
                        )
    ...
    # 5. 结果统计
    df = pd.DataFrame(
        {"ID": ids_list, "True_Label": all_labels, "Predicted": all_preds, "Prob_Pos": all_probs}
    )
