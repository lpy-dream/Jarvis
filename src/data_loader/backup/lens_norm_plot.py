...
ax.axvline(
    ...
    linestyle="--",
    linewidth=2,
    label=f"3σ Bounds ({actual_lower_bound:.0f}-{actual_upper_bound:.0f})",
)
...
if max_plot_x > actual_upper_bound:
    ax.axvspan(
        actual_upper_bound, max_plot_x, color="red", alpha=0.15, label="Excluded Region (>3σ)"
    )
...
ax.set_title(
    f"Sequence Length Distribution vs. 3σ Normal Bounds\n(Total: {len(lengths):,} seqs)",
    fontsize=14,
)
